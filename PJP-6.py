...
"""
================================================================================
UNIFIED TOOL: PJP (256+ transforms, 2704 pairs, quantum, dictionaries)
             + paq8px 5‑step installer + paq8px CLI compress/decompress
             + paq & zstandard backends for PJP
================================================================================
Usage:
  1) Interactive PJP menu (default):
         python pjp_tool.py
  2) Install paq8px from source (5 steps):
         python pjp_tool.py --install-paq8px
  3) Compress a file with paq8px (after installation):
         python pjp_tool.py --paq-compress input_file output_file.paq8px
  4) Decompress a file with paq8px:
         python pjp_tool.py --paq-decompress input_file.paq8px output_file
  5) PJP also uses Python 'paq' and 'zstandard' modules if installed.
================================================================================
"""
...
# ------------------------------------------------------------------
# Automatic paq8px installer (5 steps)
# ------------------------------------------------------------------
def install_paq8px():
    """Five‑step automatic download, compile and install of paq8px."""
    REPO_URL = "https://github.com/hxim/paq8px/archive/refs/heads/master.zip"
    INSTALL_DIR = Path.home() / ".local" / "bin"

    with tempfile.TemporaryDirectory() as tmpdir:
        tmpdir_path = Path(tmpdir)
        zip_dest = tmpdir_path / "paq8px.zip"

        # Step 1 – Download
        print("Step 1/5: Downloading paq8px source from GitHub...")
        urllib.request.urlretrieve(REPO_URL, zip_dest)
        ...
        # Find the extracted directory (usually paq8px-master)
        source_dir = None
        for d in tmpdir_path.iterdir():
            if d.is_dir():
                source_dir = d
                break
        ...
        if (source_dir / "Makefile").exists():
            ...
            sources = " ".join(str(f) for f in cpp_files)
            cmd = f"g++ -O3 -march=native -std=c++11 {sources} -o paq8px -lz"
            subprocess.run(cmd, shell=True, check=True)
        print("  Compilation successful.")

        binary = source_dir / "paq8px"
        if not binary.exists():
            raise RuntimeError("Binary not found after compilation.")

        # Step 4 – Install
        print(f"Step 4/5: Installing to {INSTALL_DIR}")
        INSTALL_DIR.mkdir(parents=True, exist_ok=True)
        dest = INSTALL_DIR / "paq8px"
        import shutil
        shutil.copy(binary, dest)
        dest.chmod(0o755)

        # Update PATH if using bash/zsh
        shell_config = None
        ...
        try:
            result = subprocess.run(["paq8px", "-h"], capture_output=True, text=True, timeout=10)
            if result.returncode == 0:
                print("  Success! paq8px is working.")
            else:
                print("  Warning: paq8px returned non‑zero exit code, but may still work.")
        except FileNotFoundError:
            print("  paq8px not found in PATH. Please open a new terminal or restart your shell.")
        print("✅ Installation complete.")

# ------------------------------------------------------------------
# paq8px command-line wrappers
# ------------------------------------------------------------------
def run_paq8px_compress(infile: str, outfile: str, level: str = "-8"):
    """Compress a file with the installed paq8px."""
    cmd = ["paq8px", level, outfile, infile]
    print(f"Running: {' '.join(cmd)}")
    try:
        subprocess.run(cmd, check=True)
        print(f"Compressed {infile} → {outfile} using paq8px")
    except FileNotFoundError:
        print("Error: paq8px not found. Install it with --install-paq8px first.")
    except subprocess.CalledProcessError:
        print("paq8px compression failed.")

def run_paq8px_decompress(infile: str, outfile: str):
    """Decompress a paq8px archive."""
    cmd = ["paq8px", "-d", infile, outfile]
    print(f"Running: {' '.join(cmd)}")
    try:
        subprocess.run(cmd, check=True)
        ...
        print("paq8px decompression failed.")

# ------------------------------------------------------------------
# PJP globals & import attempts
# ------------------------------------------------------------------
USE_QUANTUM = False
HAS_QISKIT = False
...
# Try importing Python backends (paq and zstd)
try:
    import paq
except ImportError:
    paq = None
...
class PJPCompressor:
    ...
    def decompress_file(self, infile: str, outfile: str):
        ...
        if len(data) >= 4 and data[:4] == b'\x00c\x00\x00':
            print("Detected old paq magic – stripping and decompressing with paq...")
            if paq is None:
        ...
        if len(data) >= 4 and data[:4] == b'(\xb5/\xfd':
            print("Detected old zstd magic – stripping and decompressing with zstd...")
            if not HAS_ZSTD:
...
def main():
    ...
    if len(sys.argv) > 1:
        cmd = sys.argv[1]
        if cmd == "--install-paq8px":
            install_paq8px()
            return
        elif cmd == "--paq-compress":
            if len(sys.argv) < 4:
                print("Usage: python pjp_tool.py --paq-compress <input> <output.paq8px>")
                return
            run_paq8px_compress(sys.argv[2], sys.argv[3])
            return
        elif cmd == "--paq-decompress":
            if len(sys.argv) < 4:
                print("Usage: python pjp_tool.py --paq-decompress <input.paq8px> <output>")
                return
            run_paq8px_decompress(sys.argv[2], sys.argv[3])
            return
        else:
            print("Unknown flag. Use --install-paq8px, --paq-compress, --paq-decompress, or run without flags for interactive PJP menu.")
            return

    # Interactive PJP menu
