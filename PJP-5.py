...
+ Backend markers: 0xFE for zstd, 0xFD for paq, 0xFC for raw (only when needed)
============================================================================
"""
...
class PJPCompressor:
    ...
    #   0xFE = zstd,  0xFD = paq,  0xFC = raw (only when needed)
    # ------------------------------------------------------------------
    def _compress_backend(self, data: bytes, safe: bool = False) -> bytes:
        ...
        if HAS_ZSTD:
            try:
                ...
                pass
        # raw: no marker for safe (we add 0xFC separately in the header), but for consistency we use 0xFC as payload marker
        # However, to save bytes, we will not add a marker for raw; the header (0xFC) already indicates raw.
        # But for safe mode (dictionary), we need a marker to identify raw. We'll use 0xFC as raw marker.
        if safe:
            candidates.append((b'\xFC', data))   # raw marker 0xFC
        else:
        ...
        if not safe:
            # non‑safe: we return the payload without any extra marker (except the backend marker if used)
            # In non‑safe, the candidates already include the marker (b'Z', b'L', b'N')
            _, best = min(candidates, key=lambda x: len(x[1]))
            return best
        else:
            # safe: we want to include the marker in the payload
            _, best = min(candidates, key=lambda x: len(x[1]))
            return best
    ...
    def _decompress_backend(self, data: bytes, safe: bool = False) -> Optional[bytes]:
        ...
        if paq is not None:
            try:
                ...
                pass
        # if data starts with 'N' (raw marker from non‑safe), strip it
    ...
    def compress_with_best(self, data: bytes, safe: bool = False, ultra: bool = True,
        ...
        # In safe mode, raw_backend already contains the 0xFC marker, but we also add the header 0xFC.
        # To avoid double 0xFC, we should not add a marker for raw if we already have the header.
        # Actually, we want the output to be header + payload. For raw, payload should be just the data (no extra marker) to save bytes.
        # But our _compress_backend adds a marker for safe mode. Let's change that: for raw, we will not add a marker in _compress_backend,
        # and we will add the 0xFC header to indicate raw. So we need to adjust _compress_backend to not add a marker for raw when safe.
        # I'll modify _compress_backend: for safe, raw candidate returns just data (no marker), and we add the header 0xFC separately.
        # But currently, _compress_backend always returns a payload with marker. To keep it simple, we'll handle raw separately.
        # Let's refactor: in compress_with_best, we manually create raw candidate without marker.
        if safe:
            raw_candidate = self._encode_marker_raw() + data
        else:
            raw_backend = self._compress_backend(data, safe)  # contains 'N' marker for non‑safe
            raw_candidate = self._encode_marker_raw() + raw_backend
        ...
        for t in single_transforms:
            try:
                ...
                if safe:
                    # For safe, we need to include the backend marker; it's already in backend.
                    candidate = self._encode_marker_single(t) + backend
                else:
                    # For non‑safe, backend may not include marker (or includes 'Z','L','N')
                    candidate = self._encode_marker_single(t) + backend
    ...
    def _decompress_auto(self, data: bytes) -> Tuple[bytes, Optional[Tuple[int, ...]]]:
        ...
        # For safe mode, we detect the marker byte at the start of payload.
        # For non‑safe, we try backends without markers.
        first_byte = payload[0:1]
        if first_byte in (b'\xFE', b'\xFD', b'\xFC'):
            # safe mode markers
